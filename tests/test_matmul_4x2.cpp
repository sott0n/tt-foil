...
//
// v6-4: 4×2 grid block-sharded DRAM matmul (8 cores).
//
// Direct scale-up of v6-2's 2×2 layout: 4 row groups × 2 column groups
// of cores, each running an independent Mt_pc × Kt × Nt_pc matmul on
// its sub-block of the global problem.
//
// DRAM layout mirrors v6-2:
//   A : one global buffer (kGridY * Mt_pc × Kt tiles). Both cores in
//       row cy read the slice starting at cy * Mt_pc.
//   B : kGridX column buffers (Kt × Nt_pc each). All cores in column
//       cx share buf_b[cx].
//   C : kCores per-core buffers (Mt_pc × Nt_pc each). Host stitches.
...
int main() try {
    ...
    // ---- Device + 8 cores --------------------------------------------
    // Logical CoreCoord {x, y} mapped from (cy, cx) — same convention as
    // matmul_2x2 (x = cy, y = cx).
    std::vector<tt::foil::CoreCoord> open_cores;
    for (uint32_t cy = 0; cy < kGridY; ++cy)
        for (uint32_t cx = 0; cx < kGridX; ++cx)
            open_cores.push_back(tt::foil::CoreCoord{cy, cx});

    auto dev = tt::foil::open_device(pcie_index, "", open_cores);
    ...
    // Launch all 8 cores in parallel.
    std::vector<tt::foil::Kernel*> raw_kernels;
    raw_kernels.reserve(kCores);
    for (uint32_t cid = 0; cid < kCores; ++cid) raw_kernels.push_back(kernels[cid].get());
    tt::foil::execute(*dev, std::initializer_list<tt::foil::Kernel*>{
        raw_kernels[0], raw_kernels[1], raw_kernels[2], raw_kernels[3],
        raw_kernels[4], raw_kernels[5], raw_kernels[6], raw_kernels[7],
    });
    ...
    if (bad != 0) {
        std::fprintf(stderr,
            "test_matmul_4x2(per-core Mt=%u Kt=%u Nt=%u, 4x2 cores): "
        ...
        std::puts("test_matmul_4x2: FAIL");
        return 1;
    }

    std::printf("test_matmul_4x2: PASS  (per-core Mt=%u Kt=%u Nt=%u, 4x2 cores, "
    ...
    std::fprintf(stderr, "test_matmul_4x2: FAIL — %s\n", e.what());
    return 1;
}
