...
//
// HW test: ElementwiseAdd via op_lib (Transformer residual).
...
constexpr uint32_t kNT    = EADD_OP_NT;
...
int main() try {
    ...
    auto op = ol::make_eltwise_add(*dev, a, b, out);
    ol::execute(*dev, op);
    ...
    if (bad != 0) {
        std::fprintf(stderr, "test_eltwise_add_op: %u bad, worst=%.5f\n", bad, worst);
        std::fprintf(stderr, "test_eltwise_add_op: FAIL\n");
        tt::foil::close_device(std::move(dev));
        return 1;
    }
    std::printf("test_eltwise_add_op: PASS  (NT=%u, worst=%.5f)\n", kNT, worst);
    tt::foil::close_device(std::move(dev));
    return 0;
} catch (const std::exception& e) {
    std::fprintf(stderr, "test_eltwise_add_op: FAIL — %s\n", e.what());
    return 1;
}
