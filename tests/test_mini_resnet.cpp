...
//
// Mini ResNet — stem followed by two basic_blocks, end-to-end on one
// Tensix core. This is the deepest tt-foil pipeline so far: 4 distinct
// kernel programs (Conv₇ₓ₇, Maxpool₃ₓ₃, Conv₃ₓ₃, residual_add) rotated
// across 10 device executes, four of them touching the same Y buffer.
//
//   x        : (C=32, 32, 32)
...
int main() try {
    ...
    std::vector<uint16_t> y_dev;
    {
        ...
        run_add(*buf_Ym, *buf_Ys, *buf_Yo, Ym_noc, Ys_noc, Yo_noc,
                t4_tiles, y1_tiles, y_dev);
        relu_inplace(y_dev);
    }

    // ---- Compare ---------------------------------------------------
    // ~10 bf16 rounding stages stack across the chain (conv₇, bias+ReLU,
    // maxpool sel, conv₃×2 + bias×2 + sum + ReLU per block × 2 blocks).
    // Allow ~0.4 absolute and ~7 % relative.
    const float kAbsTol = 0.40f;
    const float kRelTol = 0.07f;
    ...
    if (bad != 0) {
        std::fprintf(stderr,
            "test_mini_resnet: %u/%zu mismatches; first at idx %u: "
        ...
        std::puts("test_mini_resnet: FAIL");
        return 1;
    }

    std::printf("test_mini_resnet: PASS  "
                "(stem %ux%u→%ux%u→%ux%u then 2× basic at %ux%u, C=%u, "
                "worst abs=%.5f, worst rel=%.4f%%)\n",
                kHin, kWin, kHmid, kWmid, kHpost, kWpost,
                kHpost, kWpost, kC, worst_abs, worst_rel * 100.0f);
    tt::foil::close_device(std::move(dev));
    return 0;
} catch (const std::exception& e) {
    std::fprintf(stderr, "test_mini_resnet: FAIL — %s\n", e.what());
    return 1;
}
