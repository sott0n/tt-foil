...
//
// 7×7 strided convolution — the input layer of ResNet's stem. Algorithm
// is identical to the 3×3 conv example (im2col on the host, matmul on
// the device) but with a kernel of 49 spatial positions instead of 9.
//
//   H_out = (H_in + 2*pad - K) / stride + 1
// With H_in = 16, pad = 3, stride = 2, K = 7 → H_out = 8.
//
// Tile mapping:
//   M = C_out        → Mt = C_out / 32
//   K = C_in * 49    → Kt = C_in * 49 / 32       (49 is odd, so C_in
//                                                 must be a multiple of 32
//                                                 for K to be tile-aligned)
//   N = H_out*W_out  → Nt = H_out*W_out / 32
...
constexpr uint32_t kKH = 7;
constexpr uint32_t kKW = 7;
constexpr uint32_t kPad    = 3;
constexpr uint32_t kStride = 2;
constexpr uint32_t kCin    = kKdim / (kKH * kKW);
static_assert(kCin * kKH * kKW == kKdim,
              "MM_KT*32 must equal C_in*49 (set MM_KT to C_in*49/32)");

constexpr uint32_t kWout = 8;
constexpr uint32_t kHout = kHWout / kWout;
static_assert(kHout * kWout == kHWout, "W_out must divide HW_out");

// H_in = (H_out - 1)*stride + K - 2*pad.  With pad=3, K=7, stride=2 this
// gives H_in = 2*H_out + 1; for H_out = 8 → H_in = 17. We pad inputs to
// the next-multiple-of-2 grid for a clean test: H_in = 16 gives
// H_out = (16+6-7)/2 + 1 = 8 (rounded down — the last input row falls
// off the window). So use H_in = 16 explicitly.
constexpr uint32_t kHin = 16;
constexpr uint32_t kWin = 16;
static_assert((kHin + 2 * kPad - kKH) / kStride + 1 == kHout, "H bookkeeping");
...
// Reference 7×7 stride-2 pad-3 conv (CHW layout).
void conv7x7_s2_reference(const std::vector<uint16_t>& x_chw,
                ...
                for (uint32_t ci = 0; ci < kCin; ++ci)
                    for (uint32_t ki = 0; ki < kKH; ++ki)
                        for (uint32_t kj = 0; kj < kKW; ++kj) {
                            int ih = static_cast<int>(ho * kStride + ki) - static_cast<int>(kPad);
                            int iw = static_cast<int>(wo * kStride + kj) - static_cast<int>(kPad);
...
// im2col for 7×7 stride 2: row = ci*49 + ki*7 + kj, col = ho*W_out + wo.
...
void weight_reshape(const std::vector<uint16_t>& w_cchw, std::vector<uint16_t>& w_mat) {
    w_mat.assign(kCout * kKdim, 0);
    for (uint32_t co = 0; co < kCout; ++co)
        for (uint32_t ci = 0; ci < kCin; ++ci)
            for (uint32_t ki = 0; ki < kKH; ++ki)
                for (uint32_t kj = 0; kj < kKW; ++kj)
                    w_mat[co * kKdim + ci * (kKH * kKW) + ki * kKW + kj] =
...
int main() try {
    ...
    for (uint32_t ci = 0; ci < kCin; ++ci)
        for (uint32_t h = 0; h < kHin; ++h)
            for (uint32_t w_ = 0; w_ < kWin; ++w_)
                x_chw[(ci * kHin + h) * kWin + w_] = f32_to_bf16(
                    0.005f * static_cast<float>((ci + 1) + ((h * kWin + w_) % 11)));
    for (uint32_t co = 0; co < kCout; ++co)
        for (uint32_t ci = 0; ci < kCin; ++ci)
            for (uint32_t ki = 0; ki < kKH; ++ki)
                for (uint32_t kj = 0; kj < kKW; ++kj)
                    w_cchw[((co * kCin + ci) * kKH + ki) * kKW + kj] =
                        f32_to_bf16(0.003f *
                            static_cast<float>((co + 1) + (ci % 7) + (ki * 7 + kj)));

    // ---- Reference -------------------------------------------------
    std::vector<uint16_t> y_ref;
    conv7x7_s2_reference(x_chw, w_cchw, y_ref);

    // ---- im2col + weight tiling ------------------------------------
    std::vector<uint16_t> a_mat, w_mat, w_tiles, a_tiles;
    ...
    std::array<uint32_t, 3> ra_ncrisc = { lo(out_noc), hi(out_noc), kMt * kNt };
    ...
    untile_matrix(y_tiles, kMt, kNt, kHWout, y_dev);
    ...
    if (bad != 0) {
        std::fprintf(stderr,
            "test_conv_7x7 (C_in=%u C_out=%u %ux%u→%ux%u): %u/%zu mismatches; "
            "first at idx %u: got=%.5f expected=%.5f, worst abs=%.5f, worst rel=%.4f%%\n",
        ...
        std::puts("test_conv_7x7: FAIL");
        return 1;
    }

    std::printf("test_conv_7x7: PASS  (C_in=%u C_out=%u %ux%u→%ux%u, "
                "K=7 pad=3 stride=2, worst abs=%.5f, worst rel=%.4f%%)\n",
                kCin, kCout, kHin, kWin, kHout, kWout,
                worst_abs, worst_rel * 100.0f);
    tt::foil::close_device(std::move(dev));
    return 0;
} catch (const std::exception& e) {
    std::fprintf(stderr, "test_conv_7x7: FAIL — %s\n", e.what());
    return 1;
}
