...
//
// op_lib: EltwiseAddOp — Transformer residual connection.
// Shares reader/writer with eltwise_mul; only the compute ELF prefix
// differs ("add.trisc{0,1,2}.elf" vs "mul.trisc{0,1,2}.elf").
...
EltwiseAddOp make_eltwise_add(tt::foil::Device& dev,
    ...
    if (a.num_tiles == 0 || a.num_tiles != b.num_tiles)
        throw std::runtime_error("op_lib::make_eltwise_add: a/b num_tiles mismatch");
    if (out.num_tiles == 0)
        out = allocate_tensor_dram(dev, a.num_tiles);
    else if (out.num_tiles != a.num_tiles)
        throw std::runtime_error("op_lib::make_eltwise_add: out.num_tiles != a.num_tiles");
    ...
    std::array<R, 5> bins = {{
        ...
        {R::RiscId::TRISC0, dir + "/add.trisc0.elf"},
        {R::RiscId::TRISC1, dir + "/add.trisc1.elf"},
        {R::RiscId::TRISC2, dir + "/add.trisc2.elf"},
    }};
    ...
    set_eltwise_add_args(dev, op, a, b, out);
    return op;
}
