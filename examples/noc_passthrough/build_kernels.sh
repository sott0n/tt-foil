...
#
# Build pre-compiled BRISC kernel ELFs for the noc_passthrough example.
# Two kernels: producer (logical core 0,0) and consumer (logical core 0,1),
# both BRISC. They communicate via NOC unicast (producer writes sum + flag
# to consumer's L1, consumer busy-loops on flag, multiplies sum by 2).
#
# Required env vars:
...
#
# Produces:
#   prebuilt/producer.elf
#   prebuilt/consumer.elf
...
build_one brisc 0 "$HERE/kernels/producer.cpp" producer
build_one brisc 0 "$HERE/kernels/consumer.cpp" consumer
