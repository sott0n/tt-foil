...
# matmul_2x2 example (v6-2).  Source files are bit-identical to
# matmul_dram — the same DRAM-backed Mt×Kt × Kt×Nt matmul kernel runs
# on each of four Tensix cores arranged in a 2×2 grid, with per-core
# RTAs pointing at different (A row slice, B column slice, C sub-block)
# in DRAM.
#
