...
# matmul_2core example (v6-1).  Source files are bit-identical to
# matmul_dram — the same DRAM-backed Mt×Kt × Kt×Nt matmul kernel runs
# on each of two Tensix cores in parallel, with per-core RTAs pointing
# at different A / C slices in DRAM (host-side row-shard).
#
# The MM_MT here is the PER-CORE slice height, not the global Mt. The
# test sets up 2 cores so total M = 2 × per-core M. Default is
# MM_MT=1 (Mt'=1 per core; total Mt=2 across 2 cores = 64 row), MM_KT=4
# (K=128), MM_NT=2 (N=64).
#
# Required env vars:
