...
# matmul_4x2 example (v6-4): an 8-core demo on a 4×2 grid. Source
# files are bit-identical to matmul_dram — the same DRAM-backed
# Mt×Kt × Kt×Nt matmul kernel runs on each of eight Tensix cores,
# with per-core RTAs pointing at different (A row slice, B column
# slice, C sub-block) in DRAM. This is v6-2's pattern scaled up by
# doubling the grid in the M direction.
#
# MM_MT and MM_NT are the PER-CORE slice height and width, not global.
# The test sets up a 2×2 grid so total Mt = 2 × MM_MT, total Nt =
# 2 × MM_NT. Default is MM_MT=1 MM_KT=4 MM_NT=1 (global 64×128 × 128×64,
# matching v6-1's default total problem size, but now split 4 ways).
#
# Required env vars:
...
EOF
    ...
    for i in 0 1 2; do
        ...
        "$GXX" "${COMMON_CFLAGS[@]}" \
            ...
            -DMM_MT=${MM_MT:-1} -DMM_KT=${MM_KT:-4} -DMM_NT=${MM_NT:-1} \
