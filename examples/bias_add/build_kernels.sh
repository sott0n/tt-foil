...
# example (v5.7).  Row-broadcast eltwise add: c_0 carries the M×N input
# tile, c_1 carries a "1×N bias" tile (only row 0 used; LLK replicates
# row 0 across M rows via BroadcastType::ROW).  Output to c_16.
# This is the broadcast-add primitive used by Linear / FC layers.
#
# Required env vars:
