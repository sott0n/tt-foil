...
# conv_3x3_2core example (v7-3): sharded 3×3 conv across two Tensix
# cores. The output's H axis is split in half; each core handles
# H_local = H/2 output rows.
#
# The halo (boundary input rows the conv needs from the neighbor) is
# staged on the host: each core's im2col matrix already contains its
# neighbor's row at the appropriate kernel offsets. So the device-side
# matmul kernel is bit-identical to matmul_dram / conv_3x3; only the
# per-core problem shape differs.
#
# MM_MT/MM_KT/MM_NT here are the PER-CORE tile counts. The test
# computes global Mt = MM_MT, Kt = MM_KT (shared K), Nt = 2 * MM_NT.
#
# Tile mapping (matmul (M, K, N) ↔ conv (C_out, C_in·KH·KW, H_out·W_out)):
#   MM_MT = ceil(C_out          / 32)
#   MM_KT = ceil(C_in · 3 · 3   / 32)   (C_in % 32 == 0 ⇒ C_in·9 is also
#                                        tile-aligned, no K-axis padding)
#   MM_NT = ceil(H_out · W_out  / 32)
#
# v7-2 default: C_in=C_out=32, H=W=8 → MM_MT=1, MM_KT=9, MM_NT=2.
#
# Required env vars:
