...
# conv_3x3_s2 example (v7-4): bf16 3×3 convolution with stride=2,
# padding=1, single batch (N=1). The ResNet downsample primitive.
#
# Stride is a HOST-SIDE concern: the im2col matrix only contains
# output-pixel positions (h_out, w_out), each gathering input pixels
# at (h_out*stride + ki - pad, w_out*stride + kj - pad). The device
# kernel is bit-identical to matmul_dram / conv_3x3; only the
# host-side im2col loop changes.
#
# Tile mapping (matmul (M, K, N) ↔ conv (C_out, C_in·KH·KW, H_out·W_out)):
#   MM_MT = ceil(C_out          / 32)
#   MM_KT = ceil(C_in · 3 · 3   / 32)   (C_in % 32 == 0 ⇒ K is also tile-aligned)
#   MM_NT = ceil(H_out · W_out  / 32)
# v7-4 default: C_in=C_out=32, H_in=W_in=16, stride=2, pad=1 →
#   H_out=W_out=8 → MM_MT=1, MM_KT=9, MM_NT=2 (same shape as conv_3x3).
#
# Required env vars:
