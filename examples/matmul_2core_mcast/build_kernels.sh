...
# matmul_2core_mcast example (v6-3): 2-core row-shard matmul where
# the B operand is read from DRAM exactly ONCE by the producer core
# and forwarded to the consumer core over NOC, halving DRAM-side B
# bandwidth vs v6-1.
#
# Two separate reader ELFs (each BRISC):
#   reader_producer.brisc.elf — DRAM B → local L1 staging → NOC peer
#   reader_consumer.brisc.elf — busy-wait on peer flag, then local staging
# Compute and writer are bit-identical to matmul_dram.
#
# MM_MT here is the per-core slice height (same convention as v6-1).
#
# Required env vars:
...
EOF
    ...
    for i in 0 1 2; do
        ...
        "$GXX" "${COMMON_CFLAGS[@]}" \
            ...
            -DMM_MT=${MM_MT:-1} -DMM_KT=${MM_KT:-4} -DMM_NT=${MM_NT:-2} \
...
build_one brisc  0 "$HERE/kernels/reader_producer.cpp" reader_producer.brisc
build_one brisc  0 "$HERE/kernels/reader_consumer.cpp" reader_consumer.brisc
