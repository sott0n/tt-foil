...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the conv_3x3
# example (v7-2): bf16 3×3 convolution, padding=1, stride=1, single
# batch (N=1). The conv is unrolled into a matmul over the im2col'd
# activation matrix on the host. The device-side matmul kernel is
# bit-identical to matmul_dram; only the tile-count shape differs.
#
