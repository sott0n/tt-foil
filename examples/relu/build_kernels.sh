...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the relu
# example (v5.5).  Same pipeline shape as tile_copy (1 input CB, 1
# output CB), but the compute kernel runs init_sfpu + relu_tile in place
# of copy_tile to clamp negatives to 0 on the way through DST regs.
#
# Required env vars:
