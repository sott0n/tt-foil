...
# residual_add example: RA_NT-tile bf16 eltwise add of two DRAM streams.
# Same per-tile pipeline as add_tiles, wrapped in a tile loop — the
# building block for ResNet's skip-connection add.
#
# Required env vars:
...
#   RA_NT                Tile count baked into kernels (default: 8). Must
#                        match the host test's expectation.
#
# Produces:
...
# PREBUILT_DIR lets a sibling example (e.g. basic_block) reuse these
# kernel sources at a different RA_NT without clobbering this example's
# own prebuilt/.  Defaults to $HERE/prebuilt.
