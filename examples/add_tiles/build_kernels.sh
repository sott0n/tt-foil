...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the add_tiles
# example (v5.6).  Same 2-input pipeline as matmul_1tile (CB c_0 = A,
# c_1 = B, c_16 = OUT), but the compute kernel runs add_tiles instead of
# matmul_tiles — eltwise binary on a single tile.
#
# Required env vars:
