...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the conv_1x1
# example (v7-1).
#
# A 1×1 convolution with N=1 batch is mathematically a matmul:
#   Y[c_out, h, w] = sum_{c_in} W[c_out, c_in] * X[c_in, h, w]
# Reshaping X as (C_in, H*W) and using W as (C_out, C_in), the output
# Y of shape (C_out, H*W) is W @ X. So the device kernels are
# bit-identical to matmul_dram — only the host-side tensor layout
# differs.  Compile-time tile counts:
#   MM_MT = ceil(C_out / 32)   per-core if multi-core, else global
#   MM_KT = ceil(C_in  / 32)
#   MM_NT = ceil(H*W   / 32)
# v7-1's test fixes MM_MT=MM_KT=MM_NT=2 (C_in=C_out=64, H=W=8 → H*W=64).
#
# Required env vars:
