...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the matmul_kt
# example (v5-2): K-loop matmul that accumulates Kt partial products in
# DST regs (DST_ACCUM_MODE=true, fp32 DST) and packs once.
#
# Kt is baked into the TRISC compute ELF at build time via -DMM_KT=<N>.
# Override via env: MM_KT=8 ./build_kernels.sh.  Test_matmul_kt expects
# the same value.
#
# Required env vars:
