...
#
# Build pre-compiled BRISC + NCRISC + TRISC kernel ELFs for the bias_add
# example (v5.7).  Multi-tile column-broadcast bias add + optional ReLU. c_0 carries the (Mt × Nt) input
# tile, c_1 carries one (32 × 32) bias tile with channel biases in col 0; LLK replicates
# col 0 across N spatial slots via BroadcastType::COL).  Output to c_16.
# Optional ReLU controlled by a runtime arg.
#
# Required env vars:
...
# PREBUILT_DIR override lets siblings reuse this kernel under their own dir.
