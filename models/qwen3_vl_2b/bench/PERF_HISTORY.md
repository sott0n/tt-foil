...
| 2026-05-28 | iter3-matmul-opcache | (this commit) | **3.72** (3.70/3.71/3.72/3.73/3.76) | 1.37 | 0.46 | 0.91 | **matmul OpCache + disjoint pinned grids** (same mechanism as text qwen3_run's `iter-matmul-opcache`). `make_matmul_grid_cached` (`src/op_lib/matmul.cpp`) keeps per `(Kt, n_cores, first_core)` `MatMulGridOp` pinned via `pin_persistent`; first call runs the full `make_matmul_grid`, subsequent calls collapse to `set_matmul_grid_args` (RTA refresh). `qwen3vl_run.cpp` lays out three disjoint persistent grids (row 2 already hosts prefill cached ops, so matmul grids start from row 3): `kMatmulKt64Grid` (3,0..3) — qkv/o/ffn_gate+up share one entry; `kMatmulKt192Grid` (4,0..3) — ffn_down (Kt=192 ~770 KB/core); `kLmHeadGrid` (5,0..7) — 8-way lm_head. `run_matmul_on` drops the per-call `release_kernels`/`reset_l1` loop (would unpin). 5-run paired vs iter2 baseline (4.62 median): **3.70/3.71/3.72/3.73/3.76, median -900 ms (-19.5%)**. Bit-identical tokens 9640×5. Per-call wins (decode, slow-dispatch): `dec:matmul_q` 1.37→0.49 (-64%), `dec:matmul_k` 1.20→0.33 (-72%), `dec:matmul_v` 1.21→0.34 (-72%), `dec:matmul_o` 1.37→0.49 (-64%), `dec:matmul_gate` 1.83→0.96 (-48%), `dec:matmul_up` 1.83→0.95 (-48%), `dec:matmul_ffn_d` 1.98→1.11 (-44%), `dec:lm_head` 15.5→13.83 (-11%). decode 1607→911 ms (-43%), prefill 626→456 ms (-27%). VL benefits more than text qwen3_run (which gets -10.5%) because Q/K/V/gate/up are unfused (7 matmul/layer vs 3 fused), so amortizing per-call host setup compounds. **Lesson** (same as text HISTORY): post-cache per-call cost = ② worker exec floor + ③ pure compute. Iter11's "Mt=1 small-Nt matmuls don't scale past 4 cores" regression reduced from +30-45% to +1-3% but still applies — kMatmulKt64Grid kept at 4 cores. Remaining wall 3.72 s: weights upload 1.37 s + decode 0.91 s + prefill 0.46 s + open/teardown/host ~1.0 s. `make_matmul_grid_cached` API shared with text qwen3_run. |
| 2026-05-27 | iter2-prefill-device-kv-snapshot | (prev commit) | **4.62** (4.61/4.62/4.62/4.63) | 1.37 | 0.62 | 1.61 | **Added `ops/kv_snapshot/` (BRISC-only) to replace host-side kv_cache_snapshot in prefill.** The kernel takes block-major K^T from `pre:transpose` (`T_Kt_pre`, tile (c,s) at `(c*kSt+s)*2048`) and slot-major V from the gqa-attention path (`T_V_pre`, tile (s,c) at `(s*kNkDt+c)*2048`), slurps both into L1 with one NOC read each, re-positions K^T tiles to slot-major `(s*kNkDt+c)*2048` in `T_Kt_cache`, copies V tiles to `T_V_cache`, and zero-fills decode slots (kSt..kStKv) with a reused 2 KB L1 zero tile. `make_kv_snapshot` caches kSt/kNkDt/kStKv via OpCache; runs 28 layers × 1 dispatch on dedicated core (1,6) `kPreKvSnapCore` (booted at open_device). Profile vs paired iter1: pre:kv_cache_snapshot(host) **455 ms (28×16.26)** → **pre:kv_snapshot 5.7 ms (28×0.20)** = **-98.7%**. Prefill total 1077 → 624 ms (-453 ms, -42%). Wall 5.07 → 4.62 s (-450 ms, **-8.9%**). Bit-identical tokens 9640×5. LOC ~210 (kernel 117, op_lib 113, qwen3vl_run -30+10, header 21, CMakeLists 1). Removed host helper `kt_to_slot_major` and related constants. **Bench noise note**: 1 of 5 consecutive runs hit 9.96 s (all phases ×2 — thermal/contention spike); the 3 post-cooldown runs (4.61/4.62/4.62) were stable at 20 ms spread. Allow 1 outlier and use median when pairing. **Cumulative iter1+iter2**: wall 5.28 → 4.62 s (-660 ms, **-12.5%**), prefill 1.08 → 0.62 s (-42%); both host hot bars resolved. Remaining: weights load+upload 1.37 s (26%), decode 1.61 s (31%, all ops at per-core membar dispatch floor). |
