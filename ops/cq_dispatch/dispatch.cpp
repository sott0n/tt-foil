...
//       * CMD_LAUNCH_BATCH→ fire N workers' GOs, then poll all DONE
//       * CMD_NOTIFY_HOST → bump completion_counter
//       * CMD_EXEC_TRACE  → read N cmds from a DRAM trace buffer and replay
//                            each through process_cmd (no nesting)
...
constexpr uint32_t CMD_EXEC_TRACE   = 5;
...
// Process one command located at `cmd_p` (an L1 address cast to a pointer).
// Handles LAUNCH / LAUNCH_BATCH / NOTIFY — the ops valid both inside the ring
// and inside a trace. TERMINATE and EXEC_TRACE are handled in the ring loop.
static void process_cmd(uint32_t state_addr, volatile uint32_t* cmd_p) {
    ...
    const uint32_t cmd_l1 = reinterpret_cast<uint32_t>(cmd_p);
    ...
    if (op == CMD_LAUNCH) {
        ...
        *go_msg_scratch = go_val;
        ...
        // Poll worker's GO_MSG.signal until RUN_MSG_DONE (=0). Signal byte
        // sits at byte offset 3 (high byte) of the 4-byte mailbox word.
        while (true) {
...
void kernel_main() {
    // BRISC firmware only initialises `noc_index` (=0 by default). The
    // dispatcher uses NOC 1 for peer-Tensix writes (CLAUDE.md invariant:
    // BRISC peer L1 writes hang on NOC 0), so init NOC 1's local state here.
    ...
    while (running) {
        ...
        while (rd != wptr_snap) {
            ...
            if (op == CMD_TERMINATE) {
                ...
                *rdptr_p = rd;
                break;
            } else if (op == CMD_EXEC_TRACE) {
                const uint32_t num_cmds = cmd_p[1];
                const uint64_t dram =
                    (static_cast<uint64_t>(cmd_p[3]) << 32) | cmd_p[2];
                volatile uint32_t* const tc =
                    reinterpret_cast<volatile uint32_t*>(state_addr + kTraceScratchOffset);
                for (uint32_t i = 0; i < num_cmds; ++i) {
                    noc_async_read_one_packet(
                        dram + static_cast<uint64_t>(i) * kCmdSlotBytes,
                        reinterpret_cast<uint32_t>(tc),
                        kCmdSlotBytes, kDispatchNoc);
                    noc_async_read_barrier(kDispatchNoc);
                    if (tc[0] == CMD_EXEC_TRACE) continue;  // no nesting
                    process_cmd(state_addr, tc);
                }
            } else {
                process_cmd(state_addr, cmd_p);
            }
