...
#
# Build KvAppend op kernel ELF. BRISC-only — reads T_Kr / T_V into L1,
# appends them into the per-layer K^T cache (slot1, via face read-modify
# -write) and V cache (slot1, direct strided writes) on device.
#
# Produces:
#   prebuilt/append.brisc.elf
...
build_one brisc 0 "$HERE/append.cpp" append.brisc
