...
#
# Build KvSnapshot op kernel ELF. BRISC-only — reads T_Kt_pre / T_V_pre
# into L1 then writes them out slot-major into the per-layer K^T / V
# cache (28 prefill snapshots replacing the host pre:kv_cache_snapshot
# loop).
#
# Produces:
#   prebuilt/snapshot.brisc.elf
...
BUILD="${BUILD:-/tmp/tt_foil_build_kv_snapshot}"
...
build_one brisc 0 "$HERE/snapshot.cpp" snapshot.brisc
