...
#
# Build eltwise_unary op kernel ELFs: SiLU, GELU, ReLU.
#
# All three variants share the same reader.cpp + writer.cpp; only the
# TRISC compute source differs.
#
# Required env vars:
...
#                        (default: auto-discovered like examples/)
# Optional:
#   PREBUILT_DIR         Output directory (default: <this-dir>/prebuilt)
#
# Produces (in $PREBUILT_DIR):
#   reader.brisc.elf
#   writer.ncrisc.elf
#   silu.trisc0.elf  silu.trisc1.elf  silu.trisc2.elf
#   gelu.trisc0.elf  gelu.trisc1.elf  gelu.trisc2.elf
#   relu.trisc0.elf  relu.trisc1.elf  relu.trisc2.elf
...
# Firmware discovery (same priority order as examples/).
# In a git worktree, also check the main repo's build/firmware since
# worktrees share the build output of the main checkout.
...
BUILD="${BUILD:-/tmp/tt_foil_build_eltwise_unary}"
...
build_compute() {
    ...
    local out_name="$2" # base name for ELF files: <out_name>.trisc{0,1,2}.elf

    echo "#include \"$src\"" > "$BUILD/kernel_includes.hpp"

    # Stub chlkc_list.h for bf16 unary: CB 0 (input) and CB 16 (output)
...
// Float16_b (=5) for CB 0 and CB 16; 255 (invalid) for all others
...
// bf16 32×32 tile: 4 faces of 16×16
...
# Shared dataflow kernels (one ELF serves all activation variants)
build_dataflow brisc  0 "$HERE/reader.cpp" reader.brisc
build_dataflow ncrisc 1 "$HERE/writer.cpp" writer.ncrisc

# Per-activation compute ELFs
build_compute "$HERE/compute_silu.cpp" silu
build_compute "$HERE/compute_gelu.cpp" gelu
build_compute "$HERE/compute_relu.cpp" relu
